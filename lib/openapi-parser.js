 ...
 *
 * Parses OpenAPI 3.0 specifications and generates Node-RED API endpoint
 * configurations. Supports importing from YAML/JSON files.
 */
 ...
 */
function openApiSchemaToJsonSchema(schema, components = {}) {
    ...
    if (schema.$ref) {
        ...
        // If resolution fails, return a permissive schema
        return { type: 'object' };
    }
    ...
    if (result.properties && typeof result.properties === 'object') {
        ...
        for (const [key, value] of Object.entries(schema.properties)) {
            result.properties[key] = openApiSchemaToJsonSchema(value, components);
        }
    }
    ...
    if (result.items) {
        if (Array.isArray(result.items)) {
            result.items = result.items.map(item => openApiSchemaToJsonSchema(item, components));
        } else {
            result.items = openApiSchemaToJsonSchema(result.items, components);
        }
    }
    ...
    for (const keyword of ['allOf', 'anyOf', 'oneOf']) {
        if (Array.isArray(result[keyword])) {
            result[keyword] = result[keyword].map(item => openApiSchemaToJsonSchema(item, components));
        }
    }

    // Handle additionalProperties
    if (result.additionalProperties && typeof result.additionalProperties === 'object') {
        result.additionalProperties = openApiSchemaToJsonSchema(result.additionalProperties, components);
    }
    ...
    // Handle nullable (convert to JSON Schema format)
    if (schema.nullable === true && result.type) {
        // In JSON Schema draft-07, nullable can be represented as type array
        if (!Array.isArray(result.type)) {
            result.type = [result.type, 'null'];
        } else if (!result.type.includes('null')) {
            result.type = [...result.type, 'null'];
        }
    }
 ...
 * @param {Object} spec - OpenAPI 3.0 specification object
 * @param {Object} options - Parsing options
