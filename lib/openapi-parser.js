...
function openApiSchemaToJsonSchema(schema, components = {}) {
    ...
    if (Array.isArray(schema)) {
        return schema.map(item => openApiSchemaToJsonSchema(item, components));
    }
    ...
    if (schema.$ref) {
        ...
        if (resolved) {
            ...
            delete siblings.$ref;
            return openApiSchemaToJsonSchema({ ...resolved, ...siblings }, components);
        }
        return { type: "object" };
    }
    ...
    for (const key of PARSER_SCHEMA_MAP_CONTAINERS) {
        if (result[key] && typeof result[key] === "object" && !Array.isArray(result[key])) {
            ...
            for (const [k, v] of Object.entries(result[key])) {
                out[k] = openApiSchemaToJsonSchema(v, components);
            }
            result[key] = out;
        }
    }
    for (const key of PARSER_SCHEMA_ARRAY_CONTAINERS) {
        if (Array.isArray(result[key])) {
            result[key] = result[key].map(item => openApiSchemaToJsonSchema(item, components));
        }
    }
    for (const key of PARSER_SCHEMA_VALUE_CONTAINERS) {
        if (result[key] && typeof result[key] === "object") {
            result[key] = openApiSchemaToJsonSchema(result[key], components);
        }
    }
