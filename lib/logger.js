...
function createLogger(options = {}) {
    ...
    // Keep a reference to the transport stream so it can be terminated on
    // redeploy; pino.transport() spawns a worker thread holding a file handle
    // that otherwise leaks for the life of the process.
    logger._transport = transport;

    return logger;
}

/**
 * Flushes and closes a logger created by createLogger(), terminating any
 * pino transport worker thread and its open file handle. Safe to call with
 * null/undefined or a fallback logger.
 * @param {Object} logger - Logger returned by createLogger()
 * @returns {Promise<void>} Resolves once the transport has ended
 */
function closeLogger(logger) {
    return new Promise((resolve) => {
        ...
        const transport = logger._transport;
        if (transport && typeof transport.end === "function") {
            let settled = false;
            const finish = () => { if (!settled) { settled = true; resolve(); } };
            // Don't hang shutdown if the worker never emits close.
            const timer = setTimeout(finish, 2000);
            if (typeof timer.unref === "function") timer.unref();
            transport.once("close", () => { clearTimeout(timer); finish(); });
            try {
                transport.end();
            } catch (err) {
                clearTimeout(timer);
