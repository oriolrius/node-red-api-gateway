...
class OpenApiGenerator {
    ...
    generate() {
        ...
        // Build paths from endpoints. operationId must be unique across the
        // whole document, but generateOperationId can collapse distinct paths
        // to the same id (e.g. /user-profile and /user/profile), so disambiguate
        ...
        for (const { info, node } of this.endpoints.values()) {
            ...
            // Build operation
            const operation = buildOperation(info, node);
            if (operation.operationId) {
                let uniqueId = operation.operationId;
                let suffix = 2;
                while (usedOperationIds.has(uniqueId)) {
                    uniqueId = `${operation.operationId}_${suffix++}`;
                }
