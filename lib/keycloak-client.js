...
class KeycloakClient extends EventEmitter {
    ...
    constructor(options = {}) {
        ...
        // Coerce clockTolerance to a number: it may arrive as a string from a
        // Node-RED editor number field, and `payload.exp + tolerance` would then
        // string-concatenate and silently disable the token-expiry check.
        this.config.clockTolerance = Number(this.config.clockTolerance) || 0;

        // Fall back to the default algorithm allow-list if none/empty supplied,
        // so an unset config doesn't reject every token.
        if (!Array.isArray(this.config.algorithms) || this.config.algorithms.length === 0) {
            this.config.algorithms = DEFAULT_KEYCLOAK_CONFIG.algorithms;
        }
    ...
    _log(level, message, context = {}) {
        if (this._logger && typeof this._logger[level] === "function") {
            this._logger[level]({ component: "keycloak-client", ...context }, message);
        }
    }
     ...
     */
    getExpectedIssuer() {
        // An explicit issuer override wins: tokens' `iss` can differ from the
        // URL the gateway uses to reach Keycloak (reverse-proxy / internal DNS).
        if (this.config.issuer) {
            return String(this.config.issuer).replace(/\/$/, "");
        }
    ...
    async _fetchJwks() {
        ...
        try {
            ...
            this._log("info", "JWKS refreshed successfully", {
                event: "jwks_refresh",
    ...
    async validateToken(token) {
        ...
        try {
            ...
            this._log("debug", "Token validation successful", {
            ...
            this._stats.failedValidations++;
            this._log("debug", "Token validation failed", {
                event: "token_validation",
                valid: false,
                error: err.message
            });
