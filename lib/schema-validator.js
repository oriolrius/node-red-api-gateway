...
function createAjv(options = {}) {
    const ajv = new Ajv({
        ...
        verbose: true,
        strict: false,
        // Do NOT register compiled schemas by their $id. The shared instance
        // survives Node-RED redeploys (module require cache), so registering a
        // $id would make the second compile of the same $id throw "schema with
        // key or id ... already exists", silently disabling that endpoint's
        // validation. Each schema here is compiled standalone, so we never need
